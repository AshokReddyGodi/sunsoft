...
if ($('#top-project1').length) {
        $('#top-project1').bxSlider({
            auto: true,
            autoControls: false,
			controls: false
        });
...
$(document).ready(function(){
    ...
    var floating_navigation_offset_top = $('#mgmenu1').offset().top;
             floating_navigation_offset_top+=300;
...
resizeHandler();
	...
	if($(window).width()>'768'){
        if (scroll_top > floating_navigation_offset_top) { 
            $('#mgmenu1').css({ 'position': 'fixed','display': 'block', 'top':0,'margin-top':0,'width':'89.7%'});
        } else {
            $('#mgmenu1').css({ 'position': 'relative','display': 'none','margin-top':0,'width':'100%'}); 
        }
	}else{
	     $('#mgmenu1').css({ 'position': 'relative','display': 'none','margin-top':0,'width':'100%'}); 
	}
